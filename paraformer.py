#!/usr/bin/env python3
"""
使用阿里 Paraformer 模型识别音频并生成 ASS 字幕
中文识别效果好，兼容 transformers
"""
...
def paraformer_to_ass(audio_path, output_path=None):
    """使用 Paraformer 识别并生成 ASS 字幕"""
    import torch
    ...
    # 模型路径
    model_path = os.environ.get("PARAFORMER_MODEL", "E:\\cuda\\paraformer")
    
    print(f"加载 Paraformer 模型...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='使用阿里 Paraformer 识别音频并生成 ASS 字幕')
    ...
    if args.audio is None:
        print("用法:")
        print("  python paraformer.py 音频.mp3")
        print("  python paraformer.py 音频.mp4")
        print("  python paraformer.py 音频.mp3 -o 输出.ass")
        print()
        print("安装依赖:")
        print("  pip install torch transformers librosa soundfile")
        print()
        print("下载模型:")
        print("  huggingface-cli download paraformer-zh --local-dir E:\\cuda\\paraformer")
        return
    
    paraformer_to_ass(args.audio, args.output)
